...
def show_exam_result(request, course_id, submission_id):
    ...
    for q in questions:
        correct_choices_set = set(q.choice_set.filter(is_correct=True))
        selected_choices_set = set(choices.filter(question=q))
        if correct_choices_set == selected_choices_set:
            total_grade += q.grade
        
    result = {'course': course, 'grade': total_grade, 'choices': choices}
